/**
 * Google Apps Script: タスクマスタデータ入力
 *
 * 【実行方法】
 ...
 * 3. このコードをコピー&ペースト
 * 4. 関数 populateTaskMaster() を実行
 *
 * 【前提条件】
 * - createTaskSheets() 関数を先に実行していること
 * - 「タスクマスタ」シートが存在すること
 */
...
function populateTaskMaster() {
  ...
  if (!sheet) {
    Logger.log('❌ エラー: タスクマスタシートが見つかりません');
    Logger.log('先に createTaskSheets() 関数を実行してください');
    return;
  }
  ...
  // データ書き込み（ヘッダー行の次から）
  const startRow = 2; // A2から開始（A1はヘッダー）
  const startCol = 1;  // A列
  const numRows = taskMasterData.length;
  const numCols = taskMasterData[0].length;

  const targetRange = sheet.getRange(startRow, startCol, numRows, numCols);
  targetRange.setValues(taskMasterData);

  Logger.log('========================================');
  Logger.log(`✅ ${taskMasterData.length}タスク入力完了！`);
  Logger.log('スプレッドシートID: 1yUrlBRsbvUvVLrSukQO0N-TTygBxl7DY1OqDQgGY06k');
  Logger.log('シート名: タスクマスタ');
  Logger.log('========================================');
  Logger.log('次のステップ:');
  Logger.log('1. Google Sheetsでデータを確認してください');
  Logger.log('2. マネジメントシステムダッシュボードで動作確認してください');
  Logger.log('   → http://localhost:3000/dashboard/tasks');
  Logger.log('========================================');
}
