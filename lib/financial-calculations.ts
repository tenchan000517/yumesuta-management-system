...
export async function calculateProfitAndLoss(
  ...
  fixedCostData.slice(1).forEach(row => {
    const isActive = row[0] === true || row[0] === 'TRUE';  // A列（有効フラグ）
    const amount = parseAmount(row[2]);                     // C列（金額）
    const startMonth = row[6];                               // G列（開始月）

    // 有効かつ開始月が対象月以前の固定費を計上
  ...
  return {
    fiscalYear: `${year}年度`,
    fiscalMonth: `${year}/${String(month).padStart(2, '0')}`,
    revenue,
    costOfSales,
    salaryExpenses,
    fixedCosts,
 ...
 * @param year - 対象年
 * @param month - 対象月（1-12）
 * @param cashAtBeginning - 期首現金残高（デフォルト: 前月末のB/Sから自動計算）
